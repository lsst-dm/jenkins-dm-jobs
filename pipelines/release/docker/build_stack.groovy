...
notify.wrap {
  ...
  def pandaRepo      = dockerRegistry.repo
    ...
    def run = {
      ...
      stage('build') {
        ...
        dir(buildDir) {
          ...
          image2 = docker.build("prompt-proto/${pandaRepo}", opt.join(' '))
        }
      }
      stage('push') {
        def digest = null
  ...
  def merge = {
    stage('digest'){
        ...
        docker.withRegistry(
          ...
          registryTags.each { name ->
            sh(script: """ \
              docker buildx imagetools create -t us-central1-docker.pkg.dev/prompt-proto/$pandaRepo:$name \
              $digest
              """,
              returnStdout: true)
          }
